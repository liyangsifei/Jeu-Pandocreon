...
public class ColèreDivineJour extends DeusEx {
	
	//constructeur
	public ColèreDivineJour(){
		 super();
		this.nom="ColèreDivineJour";
		this.origine = "Jour";
		Image image = null;
		//image = getImages.getPng("ColereDivine1");
		//this.setImage(image);
	}
			
	//Détruit une carte Guide Spirituel d'Origine Nuit ou Néant, 
	...
	//Les Croyants attachés reviennent au centre de la table.
	 public void utiliserCapacite(Joueur j,Partie partie){
		 ...
		 System.out.println("Utiliser Carte DeusEx: ColèreDivineJour");
		 System.out.println("Il peut détruire une carte Guide Spirituel d'Origine Nuit ou Néant, dont la capacité spéciale n'a pas effet. Les Croyants attachés reviennent au centre de la table.");
     	...
     	if(j.getNumeroJoueur() != 1){
   		  ...
   		  //Si ce joueur est un JoueurPhysique	
   		      System.out.println("Joueur " + j.getNumeroJoueur() + " choisit un joueur, entrer son numéro");
   		      n = sc.nextInt();
		...
		if(targetJoueur.getCarteGsEnjeu().size() != 0){
			...
			if(j.getNumeroJoueur() != 1){
				...
				while(FLAG == false){
					...
					System.out.println("Attention! Vous devez choisir une carte Guide Spirituel d'Origine Nuit ou Néant!");
			...
			if(gs.getOrigine()=="Nuit"||gs.getOrigine()=="Néant"){
			    ...
			    for(int i = 0 ; i < size ; i++){
				 ...
				 System.out.println("Vous n'avez pas choisi une carte Guide Spirituel d'Origine Nuit ou Néant!");
			 }	
		 }else{

...
public class  Vampire extends Croyant {
    ...
    public Vampire(){
      ...
      //Celui-ci choisit le sacrifié. La capacité spéciale du sacrifice est jouée.
    public void utiliserCapacite(Joueur j,Partie partie){
    	...
    	if(j.getNumeroJoueur() != 0){
    		...
    		System.out.println("Impose le sacrifice d'un Croyant d'un autre joueur. " +
    				"Celui-ci choisit le sacrifié. La capacité spéciale du sacrifice est jouée.");
    		n = sc.nextInt();
    		System.out.println("Joueur" + 0 + " a choisi Joueur numéro " + n);	
    		}
    	...
    	if(targetJoueur.getCarteCroyantEnjeu().size() != 0){
    		...
    		if(j.getNumeroJoueur() != 0){
    			...
    			while(FLAG == false){
        			System.out.println("Joueur " + 0 + " choisit un carte de Croyant dans l'espace personnel de Joueur " + targetJoueur.getNumeroJoueur());
            		n1 = sc.nextInt();
